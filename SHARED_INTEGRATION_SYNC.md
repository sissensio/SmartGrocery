...
| **2026-06-01 21:10** | Antigravity | `store_georeferencing_lifecycle_design.md`, `SHARED_INTEGRATION_SYNC.md` | **Gestione Ciclo di Vita, GPS & Self-Healing Negozi**: Disegnata la specifica per la localizzazione GPS a costo zero (Android Geocoder + OSM) e la promozione d'ufficio dei negozi a Certificati Globali in presenza di Partita IVA valida. Implementato il loop autonomo di Self-Healing: se i dati GPS georeferenziati differiscono da quelli storici a distanza di oltre 30 giorni, il server aggiorna silenziosamente l'anagrafica globale per tutti. Definiti i vincoli di protezione/immutabilità dei dati globali per utenti ordinari (HTTP 403) e la re-associazione atomica delle spese in caso di eliminazione negozio. | ✅ Sincronizzato & Verificato |
