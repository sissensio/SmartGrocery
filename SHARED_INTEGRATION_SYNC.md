...
### 🚨 4.10 Alert di Shrinkflation / Sgrammatura Ingannevole (Step 2)
Per recuperare gli alert storici e contestuali di riduzione di confezionamento a parità di prezzo nella spesa della casa:
* **Endpoint**: `GET /api/v1/scan/catalog/shrinkflation`
* **Headers**: `Authorization: Bearer <token>`
* **Risposta JSON (`List[ShrinkflationAlertResponse]`)**:
```json
[
  {
    "itemName": "Cioccolato Fondente",
    "brand": "Novi",
    "originalWeight": 0.20,
    "newWeight": 0.18,
    "weightReductionPercent": 10.0,
    "originalPrice": 2.00,
    "newPrice": 2.00,
    "priceIncreasePercent": 0.0,
    "detectedAt": "2026-05-28T11:50:00",
    "store": "Lidl",
    "barcode": "8009998887776"
  }
]
```

### 🔍 4.11 Verifica Shrinkflation per Singolo Barcode (Step 2)
Per verificare all'istante se un barcode inquadrato o visualizzato ha un alert di sgrammatura attivo:
* **Endpoint**: `GET /api/v1/scan/catalog/{barcode}/shrinkflation`
* **Headers**: `Authorization: Bearer <token>`
* **Risposta JSON (`ShrinkflationAlertResponse` o `null`)**: Restituisce il dettaglio dell'alert se presente, altrimenti `null` se il prodotto è pulito.
