...
| **2026-05-31 22:51** | AI Studio | `LocalBackendService.kt`, `MasterSyncWorker.kt`, `GroceryViewModel.kt`, `GlobalSettingsScreen.kt` | **Mappatura data/ora caricamento (created_at) per Privacy Co-housing**: Aggiunto il campo `created_at` sia a `LedgerSubmitRequest` che a `LedgerDetailedResponseDto`. Aggiornato `MasterSyncWorker` per mappare `created_at` nel database locale Room all'atto della ricezione degli scontrini di gruppo. Implementato l'helper `getCurrentUtcIsoString()` in `GroceryViewModel` per valorizzare il timestamp di creazione al secondo esatto all'atto del salvataggio locale (sia da scorrimento OCR sia da inserimento rapido manuale). Questo valore viene infine correttamente trasmesso in upload al backend. Risolte anche alcune discrepanze di tipi in `GroupMembersModal`. | ✅ Allineato & Compilato |
| **2026-06-01 20:15** | Antigravity | `openfoodfact_global_cache_plan.md`, `openfoodfact_backend_implementation.md`, `openfoodfact_android_integration_guide.md`, `SHARED_INTEGRATION_SYNC.md` | **Solidificazione Standard GS1 Prefisso 2**: Documentato il disciplinare ufficiale GS1 General Specifications (Sezione 4.2.1.2 - "Restricted Distribution Prefixes") garantendo al 100% l'assenza di collisioni globali per codici bilancia a peso variabile che iniziano con `20-29`. Esplicitate le sotto-classi standard europee per i supermercati (20/24 peso vs 21/22 prezzo) per facilitare lo sviluppo e la decodifica. Validati con successo tutti i 7 test suite del Master Test Runner con stato interamente verde (`[PASSED]`). | ✅ Sincronizzato & Verificato |
| **2026-06-01 21:10** | Antigravity | `store_georeferencing_lifecycle_design.md`, `SHARED_INTEGRATION_SYNC.md` | **Gestione Ciclo di Vita, GPS, Geofencing & Self-Healing Negozi**: Disegnata la specifica per la localizzazione GPS a costo zero (Android Geocoder + OSM) e la promozione d'ufficio dei negozi a Certificati Globali in presenza di Partita IVA valida. Implementato il loop autonomo di Self-Healing a distanza di 30 giorni. Integra il **Database dei Negozi Certificati come layer primario di Geofencing**: il server sincronizza dinamicamente i geofence dei negozi certificati entro un raggio di 15 Km (rispettando il limite Android di 100 geofence attivi), inviando notifiche push Heads-Up di checkout per qualsiasi supermercato censito globalmente. Definiti i vincoli di immutabilità dei dati certificati (HTTP 403) e la re-associazione atomica delle spese in caso di eliminazione negozio. | ✅ Sincronizzato & Verificato |
| **2026-06-01 21:40** | Antigravity | `integration_roadmap_master.md`, `SHARED_INTEGRATION_SYNC.md` | **Inizializzazione Piano Master a Camere Stagne**: Creazione del file master `integration_roadmap_master.md` per coordinare lo sviluppo congiunto tra Antigravity (Backend & PM) e AI Studio (Android). Diviso lo sviluppo in 5 blocchi stagni congelabili ed indipendenti con relativi protocolli di verifica test-driven (Alembic/PostgreSQL, geocoding/slicing, geofencing dynamic sync, bulk import OFF offline, e visual analytics). | 🚦 Pronto per Blocco 1 |
