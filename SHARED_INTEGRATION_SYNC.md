...
| **2026-05-27 20:15** | Antigravity | `SHARED_INTEGRATION_SYNC.md` & `LocalBackendService.kt` (AI Studio Task) | **Risoluzione Endpoint Registrazione**: Rilevata chiamata a `/api/v1/auth/register-dummy` (ritornante 404). Allineato il client ad utilizzare la vera API `/api/v1/auth/register` con il payload `UserCreate` corretto (inclusi `full_name` e `household_name`). | ✅ Sincronizzato & Pushed |
